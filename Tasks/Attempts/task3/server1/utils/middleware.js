  ...
  if (!token) {
    ...
    jwt.verify(token, secretKey, (err, decodedUser) => {
      if (err) {
        ...
        if(sessions.some((s) => s.token === token && s.userId === decodedUser.userId)) {
        ...
        next();
          ...
          returnError(req, res, 401, "Session not valid", "This token is not allowed to use any more to access any api.")
        }
        // req.user = decodedUser;
        // console.log("user", req.user);
        // next();
